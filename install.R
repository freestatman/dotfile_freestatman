if (TRUE) {
    ...
    pkg <- nms <- c('tidyverse', 'stringr', 'plyr', 'magrittr', 'haven',
                    'styler',
    ...
    sapply(pkg <- nms, install.packages, repos='http://cran.rstudio.com')
    ...
    devtools::install <- version("Hmisc", version = "3.17-2")

    # for BEACH required packages
    # pkg <- 2inst <- c("rtf", "sas7bdat", "WriteXLS", "SASxport")
    # pkg <- 2inst <- c("rtf", "sas7bdat", "WriteXLS", "SASxport", "rJava") # rJava needs more effort to install
    # sapply(pkg <- 2inst, install.packages)
